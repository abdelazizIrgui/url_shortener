// name, ISO 3166-1 alpha-2, dial code — used to validate that the phone
// number a user submits actually matches the dial code of the country they
// selected (frontend has the same list for the dropdown UI).
...
const COUNTRIES = [
  ...
  ["Cameroon","CM","+237"],["Canada","CA","+1"],["Chad","TD","+235"],
  ["Chile","CL","+56"],["China","CN","+86"],["Colombia","CO","+57"],
  ["Comoros","KM","+269"],["Congo (DRC)","CD","+243"],["Congo (Republic)","CG","+242"],
  ["Costa Rica","CR","+506"],["Croatia","HR","+385"],["Cuba","CU","+53"],
  ["Cyprus","CY","+357"],["Czechia","CZ","+420"],["Denmark","DK","+45"],
  ["Djibouti","DJ","+253"],["Dominican Republic","DO","+1"],["Ecuador","EC","+593"],
  ...
  ["Guinea","GN","+224"],["Haiti","HT","+509"],["Honduras","HN","+504"],
...
const DIAL_CODES = new Set(COUNTRIES.map((c) => c[2]));
const byName = new Map(COUNTRIES.map((c) => [c[0], c]));

// True if `dial` is a real dial code AND it belongs to `countryName`
// (guards against e.g. picking "Morocco" but submitting a "+1" number).
const isValidCountryDial = (countryName, dial) => {
  const entry = byName.get(countryName);
  return !!entry && entry[2] === dial;
};

module.exports = { COUNTRIES, DIAL_CODES, isValidCountryDial };
